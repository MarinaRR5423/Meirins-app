...
import { View, ScrollView, TouchableOpacity, StyleSheet, Share, Modal, TextInput, ImageBackground, Image } from 'react-native';
    ...
    {weekMenuDays.map((day, i) => {
     ...
     return (
      <View key={i} style={[styles.planNutriWeekDay, isToday && styles.planNutriWeekDayToday]}>
       {/* Header del día */}
       <View style={styles.planNutriWeekDayHead}>
        <BText style={[styles.planNutriWeekDayLabel, isToday && styles.planNutriWeekDayLabelToday]}>
         {isToday ? todayLabel : DAY_NAMES_WEEK[i]}
        </BText>
        {isToday && (
         <View style={styles.planNutriTodayBadge}>
          <BText style={styles.planNutriTodayBadgeTxt}>HOY</BText>
         </View>
        )}
       </View>
       {/* Comidas */}
       <View style={{ gap: 5, marginTop: 8 }}>
        {meals.length > 0 ? meals.map((meal, j) => (
         <View key={j} style={styles.planNutriMealRow}>
          <BText style={[styles.planNutriMealTag, isToday && styles.planNutriMealTagToday]}>
           {getMealLabel(lang, meal.label || meal.id)}
          </BText>
          <BText style={[styles.planNutriWeekMeal, isToday && styles.planNutriWeekMealToday]} numberOfLines={1}>
           {meal.title}
          </BText>
         </View>
        )) : (
...
const styles = StyleSheet.create({
 ...
 planNutriWeekMenu: { gap: 8, marginBottom: 16 },
 planNutriWeekDay: { backgroundColor: 'rgba(255,255,255,0.22)', borderRadius: 14, padding: 12 },
 planNutriWeekDayToday: { backgroundColor: 'rgba(255,255,255,0.55)' },
 planNutriWeekDayHead: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
 planNutriWeekDayLabel: { fontSize: 11, fontFamily: F.bodyB, color: '#9E3C02', textTransform: 'uppercase', letterSpacing: 0.5 },
 planNutriWeekDayLabelToday: { color: '#7A2800' },
 planNutriTodayBadge: { backgroundColor: '#7A2800', borderRadius: 20, paddingHorizontal: 7, paddingVertical: 2 },
 planNutriTodayBadgeTxt: { fontSize: 9, fontFamily: F.bodyB, color: 'white', letterSpacing: 0.5 },
 planNutriMealRow: { flexDirection: 'row', alignItems: 'baseline', gap: 6 },
 planNutriMealTag: { fontSize: 10, fontFamily: F.bodyB, color: 'rgba(158,60,2,0.65)', textTransform: 'uppercase', letterSpacing: 0.3, width: 62, flexShrink: 0 },
 planNutriMealTagToday: { color: 'rgba(122,40,0,0.65)' },
 planNutriWeekMeal: { fontSize: 12, fontFamily: F.body, color: '#260E01', lineHeight: 17, flex: 1 },
 planNutriWeekMealToday: { color: '#260E01' },
