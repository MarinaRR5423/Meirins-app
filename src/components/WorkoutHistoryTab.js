...
import { ChevronLeft, ChevronRight, Check, Minus, Plus, Clock } from 'lucide-react-native';
...
// ─── Component ────────────────────────────────────────────────────────────────
export default function WorkoutHistoryTab({ activityLog = {}, lang = 'es' }) {
  const today = new Date();
  ...
  // Stats for selected month + global streak
  const stats = useMemo(() => {
    ...
    return { done, rested, topSport, sportCount, streak };
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.statsGrid}>
        ...
        <View style={[s.statCard, { backgroundColor: '#E8F7E2' }]}>
          <BText style={[s.statVal, { color: '#1a5c15' }]}>{stats.done}</BText>
          <BText style={[s.statLbl, { color: '#2a7a20' }]}>
      ...
      {monthEntries.length === 0 ? (
        ...
        <View style={s.logList}>
          {monthEntries.map(([dateKey, entry]) => {
            ...
            const status = entry.workout; // 'done' | 'skipped' | 'extra'
            ...
            const bgColor     = isDone ? '#E8F7E2' : isExtra ? '#EFF6FF' : '#F5F5F5';
            const tc          = isDone ? '#1a5c15' : isExtra ? '#1a3a6c' : '#737373';
